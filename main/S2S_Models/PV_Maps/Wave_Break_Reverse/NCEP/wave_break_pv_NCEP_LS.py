...
lead_select1 = datetime(2021, 2, 9)
lead_select2 = datetime(2021, 2, 11)
...
lat_ave1, lat_ave2 = 60, 48
lon_ave1, lon_ave2 = 291, 324
...
filename = f'NCEP_surfT_{date_str}_perturbed.nc'
nc = Dataset(filename, 'r')
# Read in time, init time, and T2M data restricted to N Hemi domain.
time_surfT, init_time_surfT = model_utils.load_modelparam(nc)[0], (model_utils.load_modelparam(nc)[-1]-timedelta(days = 1))
surfT_pert = nc.variables['t2m'][:, :, lat_ind1:lat_ind2+1, lon_ind1:lon_ind2+1]
# Close file.
nc.close()

# Now open control file for T2M.
filename = f'NCEP_surfT_{date_str}_control.nc'
nc = Dataset(filename, 'r')
...
ltm_latitude, ltm_longitude, ltm_dates, ltm_days, surfT_ltm_region = model_utils.load_model_ltm_2D('NCEP', 'surfT', [lat1, lat2], [lon1, lon2])
...
path = os.chdir(dir)
# Read in T2M ltm data and parameters
ltm_latitude_hgt, ltm_longitude_hgt, ltm_levels_hgt, ltm_dates_hgt, ltm_days_hgt, hgt_ltm_region = model_utils.load_model_ltm_3D('NCEP', 'hgt', [lat1, lat2], [lon1, lon2], level = level)
# Set level index.
...
# Save the figure.
plt.savefig(f'/share/data1/Students/ollie/CAOs/project-2021-cao/main/S2S_Models/PV_Maps/Wave_Break_Reverse/NCEP/test_{date_str}_greenland_t2m_ncep.png', bbox_inches = 'tight', dpi = 500)
...
plt.tight_layout()
# Save figure.
plt.savefig(f'/share/data1/Students/ollie/CAOs/project-2021-cao/main/S2S_Models/PV_Maps/Wave_Break_Reverse/NCEP/test_{date_str}_greenland_pv_ncep.png', bbox_inches = 'tight', dpi = 500)
